...
(function () {
   ...
   * --------------------
   * Fee Ledger View is the dedicated mobile-only UI.
   *
   * IMPORTANT:
   * Check the viewport BEFORE calling P.bootPage("feemgmt").
   * This prevents Fee Management System from initializing on mobile.
   *
   * Mobile:
   *   fee-management.html -> fee-ledger-view.html
   *
   * Desktop:
   *   fee-management.html -> normal Fee Management System
   */
  var _financeViewport =
    window.innerWidth ||
  ...
  function renderCollect(perYear, activeY) {
    ...
    $("cB").innerHTML = '<div class="acc"><div class="sbar"><div class="sn">' + esc(s.name) + '</div><div class="sm">' + esc(a.className || "") + ' · ID ' + esc(s.id) + '</div></div><div class="year-strip"><div class="year-strip-label">Academic year</div>' + chips + '</div>' +
      ...
      '<div class="pc">' +
        '<div class="pc-head"><div><span class="pc-kicker">PAYMENT ENTRY</span><h3><i class="material-icons">payments</i>Collect a Payment</h3><p class="pc-sub">Record the amount received and review its automatic fee allocation before saving.</p></div>' +
        '<div class="pc-status"><i class="material-icons">verified</i>Ready to collect</div></div>' +
        (a.canPay ? '<div class="pc-note"><i class="material-icons">info</i><span>Enter the amount once. It will auto-allocate in this order: <b>Study Materials → Old Due → Misc → Tuition → Transport</b>.</span></div>' +
        '<div class="prow">' +
          '<div class="fld"><label>Amount Received</label><div class="pc-amount-wrap"><span class="pc-currency">₹</span><input id="cA" class="in big" type="number" min="1" inputmode="numeric" placeholder="0"/></div></div>' +
          '<div class="fld"><label>Mode</label><select id="cM" class="in">' + (BOOT.modes || []).map(function (m) { return '<option>' + m + '</option>'; }).join("") + '</select></div>' +
          '<div class="fld"><label>Date</label><input id="cD" class="in" value="' + today() + '"/></div>' +
        '</div>' +
        '<div class="fld pc-remarks"><label>Remarks <span style="font-weight:600;text-transform:none;letter-spacing:0;color:#94a3b8">(optional)</span></label><input id="cRm" class="in" placeholder="Add a note if needed…"/></div>' +
        '<div class="pc-actions"><button class="btn btn-maroon pc-preview-btn" id="cPv"><i class="material-icons">visibility</i>Preview Allocation</button></div>' +
        '<div id="cCh"></div><div id="cPr"></div>' :
        '<div class="pc-note" style="margin-bottom:0"><i class="material-icons">verified</i><span>All dues cleared for <b>' + esc(a.year) + '</b>. No payment is currently required.</span></div>') +
        '</div></div>';
  ...
  function css() {
    ...
    var styles = [
      ...
      ".pc{margin-top:18px;padding:20px;background:linear-gradient(180deg,#fff 0%,#fffdfd 100%);border:1px solid #eadede;border-radius:18px;box-shadow:0 8px 24px rgba(15,23,42,.045);position:relative;overflow:hidden}.pc:before{content:'';position:absolute;left:0;top:0;bottom:0;width:4px;background:var(--maroon)}.pc-head{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;margin-bottom:16px}.pc-kicker{display:block;margin-bottom:3px;font-size:10px;font-weight:800;letter-spacing:1.05px;color:var(--maroon)}.pc h3{margin:0;color:#202638;font-size:20px;letter-spacing:-.25px;display:flex;align-items:center;gap:8px}.pc h3 i{color:var(--maroon);font-size:21px}.pc-sub{margin:5px 0 0;color:var(--text-muted);font-size:12px}.pc-status{display:inline-flex;align-items:center;gap:6px;padding:7px 10px;border-radius:999px;background:#f6faf8;border:1px solid #dceee4;color:#166534;font-size:10.5px;font-weight:800;white-space:nowrap}.pc-status i{font-size:15px}.pc-note{display:flex;align-items:flex-start;gap:9px;margin-bottom:15px;padding:11px 13px;background:#faf6f6;border:1px solid #eee0e0;border-radius:12px;color:#64748b;font-size:11.5px;line-height:1.45}.pc-note i{color:var(--maroon);font-size:18px;margin-top:1px}.pc-note b{color:#334155}.prow{display:grid;grid-template-columns:1.35fr .8fr .8fr;gap:12px;align-items:start}.pc .fld{margin:0}.pc .fld label{display:flex;align-items:center;gap:5px;margin-bottom:5px;font-size:10.5px;font-weight:800;letter-spacing:.35px;text-transform:uppercase;color:#687386}.pc .in{min-height:44px;padding:10px 12px;border-radius:11px;background:#fff;border-color:#dfe4ea;transition:border-color .15s,box-shadow .15s,background .15s}.pc .in:focus{outline:none;border-color:#b77979;box-shadow:0 0 0 3px rgba(138,22,24,.08);background:#fff}.pc .pc-amount-wrap{position:relative}.pc .pc-amount-wrap .pc-currency{position:absolute;left:12px;top:50%;transform:translateY(-50%);font-size:15px;font-weight:800;color:var(--maroon);pointer-events:none}.pc .pc-amount-wrap #cA{padding-left:31px;font-size:21px;font-weight:800;color:#202638}.pc .pc-amount-wrap:focus-within .pc-currency{color:var(--maroon)}.pc .pc-remarks{margin-top:12px}.pc .pc-remarks .in{min-height:42px}.pc-actions{display:flex;align-items:center;justify-content:flex-end;gap:10px;margin-top:14px;padding-top:14px;border-top:1px solid #f0e8e8}.pc-preview-btn{min-height:44px;padding:11px 20px;border-radius:11px;box-shadow:0 6px 16px rgba(138,22,24,.16);justify-content:center}.pc-preview-btn:hover{transform:translateY(-1px);box-shadow:0 8px 20px rgba(138,22,24,.2)}.pc-preview-btn:active{transform:translateY(0)}.pc #cCh,.pc #cPr{margin-top:12px}.pc #cPr:empty,.pc #cCh:empty{display:none}.pc .conf{margin-top:4px;padding:14px;background:#fbfdfc;border:1px solid #dceee4;border-radius:13px}.pc .conf .ch{font-size:13px;color:#334155;margin-bottom:10px}.pc .seg{display:flex;gap:8px;flex-wrap:wrap}.pc .seg button{border:1px solid #e2caca;background:#fff;color:var(--maroon);border-radius:10px;padding:9px 13px;font:inherit;font-size:12px;font-weight:800;cursor:pointer}.pc .seg button:hover{background:#faf0f0}.pc .note{display:flex;align-items:flex-start;gap:8px;padding:10px 12px;border-radius:11px;font-size:11.5px;line-height:1.45}.pc .note i{font-size:17px}.pc .note.ok{background:#f3faf6;border:1px solid #dceee4;color:#166534}.pc .note.amber{background:#fffaf0;border:1px solid #f1dfb6;color:#8a5a00}@media(max-width:760px){.pc{padding:16px}.pc-head{flex-direction:column;gap:10px}.prow{grid-template-columns:1fr}.pc-actions{justify-content:stretch}.pc-preview-btn{width:100%}}",
