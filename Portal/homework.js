...
(function () {
  ...
  function renderMgmt(classes) {
    ...
    c.innerHTML = classes.map(function (cls) {
      ...
      var shareBtn = (session.role === "Tutor") ? "" : '<a class="btn btn-whatsapp" href="https://wa.me/?text=' + encodeURIComponent(shareTxt) + '" target="_blank" rel="noopener" style="width:auto;padding:8px 14px;font-size:13px;"><i class="material-icons" style="color:#fff;">share</i> Share Class Diary</a>';
    ...
    Array.prototype.forEach.call(c.querySelectorAll("[data-remind]"), function (b) { b.addEventListener("click", function (e) { e.stopPropagation(); var parts = b.getAttribute("data-remind").split("|"); window.open("https://wa.me/" + parts[0] + "?text=" + parts[1], "_blank"); }); });
  }
  ...
  function openBulk() {
    ...
    var rows = Object.keys(map).map(function (k) {
      ...
      return '<div class="bulk-list-item"><div style="flex:1;min-width:150px;"><strong>' + esc(t.name) + '</strong><div style="font-size:12px;color:var(--danger);font-weight:500;margin-top:2px;">Missing: ' + t.items.length + ' units</div></div>' +
        ...
        '<a class="btn btn-whatsapp" href="https://wa.me/' + t.phone + '?text=' + encodeURIComponent(msg) + '" target="_blank" rel="noopener" style="width:auto;padding:8px 12px;font-size:13px;height:36px;"><i class="material-icons" style="font-size:16px;color:#fff;">send</i> Alert</a></div></div>';
    }).join("");
