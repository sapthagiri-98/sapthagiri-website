...
(function () {
  ...
  // Opens the WhatsApp DESKTOP app (not the browser). No phone = chat picker.
  function waLink(phone, message) {
    var p = String(phone || "").replace(/\D/g, "");
    if (p.length === 10) p = "91" + p;              // add India code if missing
    return "whatsapp://send?" + (p ? "phone=" + p + "&" : "") + "text=" + encodeURIComponent(message || "");
  }
  ...
  /* ---------------- api: routed fetch() to the right backend ---------------- */
  function rpc(fn, args) {
    var attFns   = CONFIG.SUPABASE_ATT_FNS   || [];
    var smFns    = CONFIG.SUPABASE_FNS       || [];
    var ttFns    = CONFIG.SUPABASE_TT_FNS    || [];
    var hwFns    = CONFIG.SUPABASE_HW_FNS    || [];
    var usersFns = CONFIG.SUPABASE_USERS_FNS || [];   // Users Management
    var saFns    = CONFIG.SUPABASE_SA_FNS    || [];   // Staff Attendance (biometric)
    var sylFns   = CONFIG.SUPABASE_SYL_FNS   || [];   // Syllabus Tracker
    var examFns  = CONFIG.SUPABASE_EXAM_FNS  || [];   // Examinations
    var feeFns = CONFIG.SUPABASE_FEE_FNS || [];
    var dashFns = CONFIG.SUPABASE_DASH_FNS || [];
    var base = null;

    if (attFns.indexOf(fn) >= 0 && CONFIG.SUPABASE_ATT_BASE) base = CONFIG.SUPABASE_ATT_BASE;
    else if (smFns.indexOf(fn) >= 0 && CONFIG.SUPABASE_FN_BASE) base = CONFIG.SUPABASE_FN_BASE;
    if (!base && ttFns.indexOf(fn)    >= 0 && CONFIG.SUPABASE_TT_BASE)    base = CONFIG.SUPABASE_TT_BASE;
    if (!base && hwFns.indexOf(fn)    >= 0 && CONFIG.SUPABASE_HW_BASE)    base = CONFIG.SUPABASE_HW_BASE;
    if (!base && usersFns.indexOf(fn) >= 0 && CONFIG.SUPABASE_USERS_BASE) base = CONFIG.SUPABASE_USERS_BASE;
    if (!base && saFns.indexOf(fn)    >= 0 && CONFIG.SUPABASE_SA_BASE)    base = CONFIG.SUPABASE_SA_BASE;
    if (!base && sylFns.indexOf(fn)   >= 0 && CONFIG.SUPABASE_SYL_BASE)   base = CONFIG.SUPABASE_SYL_BASE;
    if (!base && examFns.indexOf(fn)  >= 0 && CONFIG.SUPABASE_EXAM_BASE)  base = CONFIG.SUPABASE_EXAM_BASE;
    if (!base && feeFns.indexOf(fn) >= 0 && CONFIG.SUPABASE_FEE_BASE) base = CONFIG.SUPABASE_FEE_BASE;
    if (!base && dashFns.indexOf(fn) >= 0 && CONFIG.SUPABASE_DASH_BASE) base = CONFIG.SUPABASE_DASH_BASE;
    ...
    if (base) {
      ...
      var headers = { "Content-Type": "application/json",
        ...
        "x-session-token": ((sess && sess.token) || "") };
      return fetch(base, { method: "POST", headers: headers, body: JSON.stringify({ fn: fn, args: args || [] }) })
    ...
    // ---- original Apps Script path (unchanged) ----
  ...
  /*
   * Chrome must be a singleton.
   * bootPage() can be called more than once during page/module
   * initialisation. Remove any previously rendered chrome first,
   * otherwise the school topbar/header gets duplicated.
   */
  var oldChrome = document.getElementById("pv2-site-chrome");
  if (oldChrome) oldChrome.remove();

  var oldFooter = document.getElementById("pv2-site-footer");
  if (oldFooter) oldFooter.remove();

  // Tag app (portal) pages so the mobile CSS can hide the marketing topbar,
  // footer and WhatsApp FAB — giving a clean, native app shell. The login
  ...
  var lo = document.getElementById("pv2Logout");
  if (lo) {
  ...
  /* ---------------- navigation ----------------
     Real pages: attendance (+ attendance-month & attendance-report via sub-tabs),
     homework, attendance-log, staff-attendance, holidays, syllabus, examinations,
  ...
  var SUBTABS = {
    attendance: [
      ...
      { id: "attreport",  label: "Monthly Report", icon: "summarize",            href: "attendance-report.html", mgmt: true },
      { id: "attabsent", label: "Absentee WhatsApp", icon: "chat", href: "attendance-absentee.html", mgmt: true }
    ],
    feemgmt: [
  ...
  function renderNav(activeId, session) {
    ...
    var SHORT = { home: "Home", attendance: "Attendance", homework: "Homework", attlog: "My Log", stafftrack: "Staff", holidays: "Holidays", syllabus: "Syllabus", exams: "Exams", examstrack: "Exam Track", fees: "Fees", feemgmt: "Fee Mgmt", studentmgmt: "Students", usermgmt: "Users", dashboard: "Dashboard" };
  ...
  window.Portal = {
    ...
    esc: esc, todayIso: todayIso, thisMonth: thisMonth, prettyDate: prettyDate, monthLabel: monthLabel, isSunday: isSunday, sortGrades: sortGrades, waLink: waLink
  };
